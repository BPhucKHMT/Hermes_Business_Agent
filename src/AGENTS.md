...
13. **File Deliverable Lifecycle & Dispatch (MEDIA Protocol)**: All generated deliverables (`.pptx`, `.xlsx`, `.docx`, `.pdf`, `.html`, `.png`, `.zip`) must be written to `.runtime/deliverables/<workspace>/<filename>` (never scattered in source or tracked workspace roots). After verifying the file exists on disk, agent must emit `MEDIA:<absolute_file_path>` on its own line in the final response so Hermes Gateway automatically uploads and delivers the native file attachment to the user.
14. **Executive Slide & Visual Deliverable Standards**: When creating presentation slides (`.pptx`) or HTML reports, never generate plain vertical bullet lists on plain white/beige backgrounds. Apply executive visual design (via `skills/research/scripts/render_deck.py` or format-native code): 16:9 widescreen layout (`13.333 x 7.5 inches`), Dark Slate Modern palette (`#0B132B` / `#1C2541` card boxes with `#48CAE4` / `#06D6A0` accents) or clean Swiss white, high-contrast typography, large stat metric callouts (Big Numbers), multi-column card grids, and 2x2 quadrant layouts. Always verify the file exists under `.runtime/deliverables/<workspace>/` and emit `MEDIA:<absolute-path>` for native Telegram delivery.
