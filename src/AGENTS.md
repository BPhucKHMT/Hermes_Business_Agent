...
14. **Executive Slide & Visual Deliverable Standards**: When creating presentation slides (`.pptx`) or HTML reports, never generate plain vertical bullet lists on plain white/beige backgrounds. Apply executive visual design (via `tools/presentation/deck_engine.py` or format-native code): 16:9 widescreen layout (`13.333 x 7.5 inches`), Dark Slate Modern palette (`#0B132B` / `#1C2541` card boxes with `#48CAE4` / `#06D6A0` accents) or clean Swiss white, high-contrast typography, large stat metric callouts (Big Numbers), multi-column card grids, and 2x2 quadrant layouts. Always verify the file exists and emit `MEDIA:<absolute-path>` for native Telegram delivery.
