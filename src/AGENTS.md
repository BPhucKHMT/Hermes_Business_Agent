...
16. **Autonomous Project Document Ingestion & Folder Allocation**: End users and clients do not know VPS or filesystem paths. When a user uploads a project plan, progress file, or spreadsheet and asks to track, follow, or save it without specifying a path, the agent MUST NOT ask the user for filesystem folder paths. Instead, the agent must automatically infer a clean workspace slug from the project name/document title (e.g. `docs/<project-slug>/`), create the directory under `docs/`, copy the file into `docs/<project-slug>/<filename>`, initialize the task tracker on Kanban (`kanban.db`), and present the structured roadmap and confirmed project name to the user.
