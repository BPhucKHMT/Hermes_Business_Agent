...
class AzureKeyVaultSecretStore:
    ...
    def put_json(self, name: str, value: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"key_vault_put_failed: could not store secret {safe_name}"
            ) from None
    ...
    def get_json(self, secret_ref: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"key_vault_get_failed: could not retrieve secret {safe_name}"
            ) from None
...
class LocalEncryptedSecretStore:
    ...
    def __init__(
        ...
        secret_key: str = "hermes-local-secret-key",
    ) -> None:
        import os
    ...
    def put_json(self, name: str, value: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"local_secret_put_failed: could not store secret {safe_name}"
            ) from None
    ...
    def get_json(self, secret_ref: str) -> Dict[str, Any]:
        ...
        if not (
            secret_ref.startswith("local://") or secret_ref.startswith("keyvault://")
        ):
            raise ValueError(f"invalid_secret_reference_scheme: {secret_ref}")
        ...
        if not target_file.is_file():
            raise RuntimeError(
                f"local_secret_not_found: could not retrieve secret {safe_name}"
            )
        try:
            ...
            raise RuntimeError(
                f"local_secret_get_failed: could not retrieve secret {safe_name}"
            ) from None
