...
class MailStore:
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS mail_connections (
                    ...
                    status TEXT NOT NULL CHECK (status IN ('connected', 'reconnect_required', 'revoked')),
    ...
    def add_connection(self, conn_record: MailConnection) -> None:
        with self._connect() as conn:
            ...
            if count >= MAX_CONNECTIONS_PER_PRINCIPAL:
                raise ValueError("connection_limit_exceeded: maximum 3 active connections allowed")
            ...
            conn.execute(
                ...
                (
                    ...
                    conn_record.owner_principal_id,
                    conn_record.mailbox_type.value if hasattr(conn_record.mailbox_type, "value") else str(conn_record.mailbox_type),
                    ...
                    conn_record.status.value if hasattr(conn_record.status, "value") else str(conn_record.status),
                    _utc_now_iso(),
                ),
            )
    ...
    def get_authorized_connection(self, principal_id: str, connection_id: str) -> MailConnection:
        with self._connect() as conn:
            cur = conn.execute(
                """
                SELECT * FROM mail_connections
            ...
            if r["owner_principal_id"] != principal_id and r["mailbox_type"] != "shared":
                raise PermissionError(f"principal not authorized for connection {connection_id}")
    ...
    def consume_link_request(self, request_id: str, nonce_hash: str, principal_id: str) -> OAuthLinkRequest:
        with self._connect() as conn:
            conn.execute("BEGIN IMMEDIATE;")
            cur = conn.execute("SELECT * FROM oauth_link_requests WHERE request_id = ?;", (request_id,))
            r = cur.fetchone()
            if not r:
                raise FileNotFoundError("oauth_request_not_found")
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO shared_grant_requests (
                    request_id, connection_id, requested_by, platform, chat_id, thread_id, status, expires_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
                """,
                (
                    req.request_id,
    ...
    def decide_grant_request(
        ...
        with self._connect() as conn:
            conn.execute("BEGIN IMMEDIATE;")
            cur = conn.execute("SELECT * FROM shared_grant_requests WHERE request_id = ?;", (request_id,))
            r = cur.fetchone()
            if not r:
            ...
            if approve:
                conn.execute(
                    """
                    INSERT INTO shared_grants (request_id, connection_id, platform, chat_id, thread_id)
                    VALUES (?, ?, ?, ?, ?);
                    """,
                    (request_id, r["connection_id"], r["platform"], r["chat_id"], r["thread_id"]),
                )
                conn.execute(
            ...
            return SharedGrantRequest(
                ...
                destination=Destination(platform=r["platform"], chat_id=r["chat_id"], thread_id=r["thread_id"]),
                status=new_status,
                expires_at=r["expires_at"],
    ...
    def destination_grant(self, connection_id: str, destination: Destination) -> Optional[SharedGrant]:
        with self._connect() as conn:
            cur = conn.execute(
                """
                SELECT * FROM shared_grants
                WHERE connection_id = ? AND platform = ? AND chat_id = ? AND (thread_id IS ? OR thread_id = ?)
                  AND revoked_at IS NULL;
                """,
                (connection_id, destination.platform, destination.chat_id, destination.thread_id, destination.thread_id),
            )
            r = cur.fetchone()
            if not r:
                return None
    ...
    def revoke_connection(self, principal_id: str, connection_id: str) -> MailConnection:
        with self._connect() as conn:
            conn.execute("BEGIN IMMEDIATE;")
            cur = conn.execute("SELECT * FROM mail_connections WHERE connection_id = ?;", (connection_id,))
            r = cur.fetchone()
            if not r:
            ...
            now = _utc_now_iso()
            conn.execute("UPDATE mail_connections SET status = 'revoked', revoked_at = ? WHERE connection_id = ?;", (now, connection_id))
            conn.execute("UPDATE shared_grants SET revoked_at = ? WHERE connection_id = ?;", (now, connection_id))
            return MailConnection(
                connection_id=r["connection_id"],
                owner_principal_id=r["owner_principal_id"],
                mailbox_type=MailboxType(r["mailbox_type"]),
