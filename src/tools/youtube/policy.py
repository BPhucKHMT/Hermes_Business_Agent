...
class YouTubePolicy:
    ...
    def validate_metadata(
        ...
        description: str = "",
        tags: tuple[str, ...] = (),
        privacy_status: VideoPrivacyStatus | None = None,
    ) -> None:
    ...
    def validate_video_file(
        ...
        if not skip_existence_check:
            ...
            if size > self.max_video_size_bytes:
                raise ValueError(
                    f"video_file_too_large_{size}_max_{self.max_video_size_bytes}"
                )
...
def load_youtube_policy(path: Path | str) -> YouTubePolicy:
    ...
    allowed_priv = tuple(
        VideoPrivacyStatus(s)
        for s in data.get("allowed_privacy_statuses", ["private", "unlisted", "public"])
    )
    return YouTubePolicy(
        ...
        allowed_video_extensions=tuple(
            ext.lower()
            for ext in data.get("allowed_video_extensions", [".mp4", ".mov", ".webm"])
        ),
