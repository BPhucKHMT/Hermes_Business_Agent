...
SELECT_FIELDS = [
    "chunk_id", "content", "title", "source_path", "source_url", "website_id", "page_id", "asset_id", "generation", "evidence_type",
    "document_version", "effective_date", "page_number", "section_heading", "slide_number",
...
def knowledge_search_many(client, queries: Iterable[str], access_groups: Iterable[str], top_k: int = 8,
    ...
    return EvidenceResult(status="ok", evidence=tuple(merged.values()), warnings=tuple(dict.fromkeys(warnings))) if merged else EvidenceResult(status="no_evidence")
