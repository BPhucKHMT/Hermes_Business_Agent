...
def upload_website_capture(
    ...
    if any(not str(capture.get(name, "")).strip() for name in required):
        raise ValueError("website capture identity is incomplete")

    groups = _groups(access_groups)
    website_id = str(capture["website_id"])
    page_id = str(capture["page_id"])
    generation = str(capture["generation"])

    prefix = f"websites/{website_id}/{generation}"
...
def delete_website_capture(
    ...
    if not site or "/" in site or "\\" in site or site in {".", ".."}:
        raise ValueError("invalid website id")

    prefix = f"websites/{site}/"
    if generation is not None:
        version = generation.strip()
        if not version or "/" in version or "\\" in version or version in {".", ".."}:
            raise ValueError("invalid website generation")
        prefix += f"{version}/"
    ...
    return {
        "status": "partial" if failures else "deleted",
        "website_id": site,
        "generation": generation,
        "deleted": deleted,
        "failures": failures,
