...
async def _trusted_crawl(
    ...
    session = (
        start_session(root_url, policy, scope, resolver=resolver)
        if resolver
        else start_session(root_url, policy, scope)
    )
    artifacts = runtime_root / session["session_id"]
    frontier = [session["root_url"]]
    seen = set()
    pages = []
    parent = None
    ...
    async with Crawl4AISession(session, artifacts, resolver, allow_private) as browser:
        while frontier:
            if time() >= session["deadline"]:
                break
            url = frontier.pop(0)
            route = urlsplit(url)._replace(fragment="").geturl()
            if route in seen:
                continue
            ...
            if time() >= session["deadline"]:
                raise ValueError("crawl exceeded wall-clock budget")

            session = (
                accept_observation(session, captured.event, resolver=resolver)
                if resolver
                else accept_observation(session, captured.event)
            )
            pages.append(captured.page)
            seen.add(route)
            parent = event_id
    ...
    capture = {
        ...
        "generation": f"gen-{uuid.uuid4().hex[:16]}",
        "pages": pages,
    }
    validated = (
        validate_capture(report, capture, runtime_root, resolver=resolver)
        if resolver
        else validate_capture(report, capture, runtime_root)
    )
    return {
        "session": session,
        "validated": validated,
