...
def indexer_status(client, indexer_name: str) -> Mapping[str, object]:
    ...
    return {
        "status": value,
        "error": "indexer_failed" if value in {"error", "transientfailure"} else None,
    }
...
def website_readiness(
    search_client, website_id: str, generation: str, pages: Iterable[Mapping[str, str]]
) -> Mapping[str, object]:
    pages = list(pages)
    expected = {str(page["page_id"]): str(page["canonical_url"]) for page in pages}
    expected_assets = {
        str(asset["asset_id"]) for page in pages for asset in page.get("assets", [])
    }
    ...
    rows = search_client.search(
        search_text="*",
        filter=(
            f"website_id eq '{_literal(website_id)}' and "
            f"generation eq '{_literal(generation)}'"
        ),
    ...
    rows = list(rows)
    actual = {
        (str(row.get("page_id", "")), str(row.get("source_url", "")))
        for row in rows
        if row.get("evidence_type") == "page_text" or not row.get("asset_id")
    }
    actual_assets = {
        str(row.get("asset_id"))
        for row in rows
        if row.get("asset_id")
        and row.get("evidence_type") in {"image_ocr", "image_description"}
    }
    ready = sorted(
        page_id
        for page_id, source_url in expected.items()
        if (page_id, source_url) in actual
    )
...
def website_absent(search_client, website_id: str) -> bool:
    ...
    rows = search_client.search(
        search_text="*",
        filter=f"website_id eq '{_literal(website_id)}'",
        select=["page_id"],
        top=1,
    )
...
def wait_for_indexers(
    ...
    names = tuple(
        dict.fromkeys(name.strip() for name in indexer_names if name and name.strip())
    )
    if not names or timeout_seconds <= 0 or interval_seconds < 0:
        raise ValueError(
            "indexer names, positive timeout, and non-negative interval are required"
        )
    ...
    while True:
        ...
        for name in names:
            ...
            fresh = fresh and (
                boundary is None or (started is not None and started >= boundary)
            )
