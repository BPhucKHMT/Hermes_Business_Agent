...
def normalize_public_url(value: str) -> str:
    ...
    if path != "/":
        path = path.rstrip("/")
    query = urlencode(
        sorted((k, v) for k, v in parse_qsl(parts.query, keep_blank_values=True) if k.lower() not in TRACKING_PARAMETERS),
        doseq=True,
    )
...
def accept_observation(
    ...
    if canonical not in result["canonical_urls"]:
        result["canonical_urls"].append(canonical)
    if fingerprint and fingerprint not in result["semantic_fingerprints"]:
        result["semantic_fingerprints"].append(fingerprint)
    ...
    for page in pages:
        ...
        for asset in page.get("assets", []):
            ...
            if (
                ...
                or asset.get("mime_type") not in {"image/png", "image/jpeg", "image/svg+xml", "image/webp"}
            ):
        ...
        semantic = {
            ...
            "assets": [{"asset_id": x.get("asset_id"), "source_url": x.get("source_url")} for x in assets],
