...
from web import normalize_public_url, validate_public_target
...
class _ImageParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "figure":
            ...
            item = {"source_url": values.get("src", ""), "alt": values.get("alt", ""), "caption": "",
                    "width": int(values.get("width", 0) or 0), "height": int(values.get("height", 0) or 0),
                    "in_figure": self.figure_depth > 0}
            self.images.append(item)
            if self.figure_depth: self.current.append(item)
...
def select_relevant_images(html: str, page_url: str) -> list[dict]:
    parser = _ImageParser(); parser.feed(html)
    selected = []
...
def download_asset(image: dict, artifact_dir: Path, resolver, byte_limit: int, opener=urlopen) -> dict:
    ...
    with opener(request, timeout=30) as response:
        ...
        suffixes = {"image/png": ".png", "image/jpeg": ".jpg", "image/svg+xml": ".svg", "image/webp": ".webp"}
        ...
        while True:
            ...
            if not chunk: break
            total += len(chunk)
            if total > byte_limit: raise ValueError("website asset exceeds byte budget")
            chunks.append(chunk)
    content = b"".join(chunks)
    if not content: raise ValueError("website asset is empty")
    digest = sha256(content).hexdigest()
    artifact_dir.mkdir(parents=True, exist_ok=True)
    path = (artifact_dir / ("asset-" + digest[:20] + suffixes[mime])).resolve()
    path.write_bytes(content)
    return {"asset_id": "asset-" + digest[:20], "source_url": final, "path": str(path), "sha256": digest,
            "mime_type": mime, "byte_count": total, "alt": str(image.get("alt", "")),
            "caption": str(image.get("caption", "")), "describe_image": False}


def map_crawl_result(result, requested_url: str, origin: str, artifact_dir: Path, event_id: str,
                     parent_event_id: str | None = None, resolver=None, disclosures=None, assets=None,
                     require_screenshot: bool = True) -> BrowserArtifact:
    if not _field(result, "success", False):
        raise RuntimeError(str(_field(result, "error_message", "Crawl4AI crawl failed")))
    validator = lambda value: validate_public_target(value, resolver) if resolver else validate_public_target(value)
    ...
    if disclosure_pairs:
        expanded = "\n\n".join("### %s\n%s" % (item["question"], item["answer"]) for item in disclosure_pairs)
    ...
    artifact_dir.mkdir(parents=True, exist_ok=True)
    screenshot = artifact_dir / (event_id + ".png") if screenshot_bytes else None
    markdown_path = artifact_dir / (event_id + ".md")
    html_path = artifact_dir / (event_id + ".html")
    if screenshot: screenshot.write_bytes(screenshot_bytes)
    ...
    artifact = {"schema_version": 1, "executor": "hermes-crawl4ai", "digest": digest,
                "html": str(html_path.resolve()), "markdown": str(markdown_path.resolve()),
                "markdown_kind": markdown_kind, "screenshot": str(screenshot.resolve()) if screenshot else None,
                "disclosures": disclosure_pairs, "assets": asset_records}
    artifact_path = artifact_dir / (event_id + ".artifact.json")
    artifact_path.write_text(json.dumps(artifact, ensure_ascii=False, indent=2), encoding="utf-8")
    started = time()
    finished = max(time(), started + 0.000001)
    semantic = " ".join(rendered.split()).encode("utf-8")
    event = {"event_id": event_id, "parent_event_id": parent_event_id, "capability": "trusted_crawl4ai",
             "requested_url": requested, "final_url": final, "canonical_url": canonical,
             ...
             "screenshot_bytes": len(screenshot_bytes), "semantic_fingerprint": sha256(semantic).hexdigest(),
             "links": links, "artifacts": [{"path": str(artifact_path.resolve()), "digest": digest}],
             "executor": "hermes-crawl4ai"}
    title_match = re.search(r"<title[^>]*>(.*?)</title>", html, re.I | re.S)
    page = {"event_id": event_id, "canonical_url": canonical,
            "title": re.sub(r"\s+", " ", title_match.group(1)).strip() if title_match else canonical,
            "rendered_text": rendered, "semantic_structure": [], "minimal_html": html,
            "assets": asset_records, "media": (_field(result, "media", {}) or {}).get("images", []),
...
class Crawl4AISession:
    ...
    async def capture(self, url: str, event_id: str, parent_event_id: str | None = None) -> BrowserArtifact:
        ...
        config = CrawlerRunConfig(cache_mode=CacheMode.BYPASS, scan_full_page=require_screenshot, screenshot=require_screenshot,
                                  exclude_external_images=True, process_iframes=False, remove_overlay_elements=True,
        ...
        assets, remaining = [], self.session["policy"]["crawl_budget"]["content_asset_bytes"]
        seen = set()
        for image in select_relevant_images(raw_html, str(_field(result, "url", url))):
            if image["source_url"] in seen: continue
            seen.add(image["source_url"])
            checked = validate(image["source_url"])
            item = download_asset(dict(image, source_url=checked), self.artifact_dir, resolver, remaining) if resolver else download_asset(dict(image, source_url=checked), self.artifact_dir, __import__("socket").getaddrinfo, remaining)
            assets.append(item); remaining -= item["byte_count"]
        return map_crawl_result(result, url, self.session["root_origin"], self.artifact_dir,
                                event_id, parent_event_id, self.resolver, self.disclosures, assets, require_screenshot)
