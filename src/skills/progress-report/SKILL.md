...
1. Resolve supplier, owner, due date, and registered report target. If any required field is unclear, ask exactly one short question for the most important missing field and stop. Example: `supplier didn't reply` without a supplier name → ask `Which supplier did not reply?`.
2. Ambiguous input creates zero source event, task, proposal, report mutation, outbound action, or KB sync. Never use retained KB facts to fill an unconfirmed current update.
3. Record source once only after required fields are resolved. Preview only the user-stated workstream, progress update, task, and supplier follow-up draft. Never invent supplier counts, quotes, outreach sent, owner/date, or ERC/property/legal/website/lease status.
...
11. Keep proposal scope minimal. A supplier-only update cannot change or summarize unrelated workstreams unless the user explicitly requests them and current evidence supports each claim.
