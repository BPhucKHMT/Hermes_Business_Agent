...
Assess sources per claim, not once for an entire report.

## Dimensions

- **Directness:** source contains direct evidence versus repeating another claim.
...
Do not silently choose one version. Show:

1. the competing claims;
2. source dates and definitions;
3. method, scope, and incentive differences;
4. which evidence is more direct or current;
5. unresolved uncertainty and confidence.

## Confidence

Use `high`, `medium`, or `low` with a short rationale:

- **High:** direct, current, independently corroborated, no material unresolved contradiction.
- **Medium:** useful direct evidence but limited independence, freshness, or scope.
- **Low:** indirect, single-source, stale, method unclear, or materially contradicted.

Confidence is not model intuition. It must cite evidence-quality factors.
