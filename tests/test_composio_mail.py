...
def test_mail_search_unauthenticated():
    with patch(
        "src.tools.composio.mail_tools.check_connection_status", return_value=False
    ):
...
def test_mail_search_authenticated():
    ...
    with (
        patch(
            "src.tools.composio.mail_tools.check_connection_status", return_value=True
        ),
        patch(
