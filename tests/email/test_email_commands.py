...
def test_handle_connect_gmail_delegates_to_composio():
    ...
    with patch(
        "tools.composio.commands.handle_connect_google",
        return_value="https://connect.composio.dev/link/test",
    ):
...
def test_handle_mail_status_delegates_to_composio():
    ...
    with patch(
        "tools.composio.commands.handle_google_status",
        return_value="Trạng thái: ĐÃ KẾT NỐI",
    ):
...
def test_handle_disconnect_gmail_delegates_to_composio():
    ...
    with patch(
        "tools.composio.commands.handle_disconnect_google",
        return_value="Đã ngắt kết nối thành công!",
    ):
