...
def test_calendar_list_unauthenticated():
    with patch(
        "src.tools.composio.calendar_tools.check_connection_status", return_value=False
    ):
        res = composio_calendar_list_events(7275339077)
        assert res["status"] == "error"
        assert (
            "chưa kết nối" in res["message"].lower()
            or "not connected" in res["message"].lower()
        )
...
def test_calendar_list_events_success():
    ...
    mock_session.execute.return_value = {
        "items": [
            {
                "id": "ev_1",
                "summary": "Họp chiến lược Protein Bar",
    ...
    with (
        patch(
            "src.tools.composio.calendar_tools.check_connection_status",
            return_value=True,
        ),
        patch(
            "src.tools.composio.calendar_tools.get_composio_client",
