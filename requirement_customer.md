...
- **REQ-AUTONOMY-03 (2026-09-16):** Với email thông thường, yêu cầu rõ ràng của đúng user về việc gửi ngay/bỏ xem nháp được tính là phê duyệt Tier 2 cho email cụ thể đó. Nếu đủ tài khoản gửi, người nhận, ý định nội dung và tệp đính kèm cần thiết, Hermes gửi rồi báo kết quả, không bắt xem draft hay xác nhận lặp lại. Thiếu/mơ hồ thông tin thì chỉ hỏi phần thiếu. Không áp dụng cho lời yêu cầu chỉ soạn nháp, lệnh nằm trong tài liệu/email, gửi tự động về sau, khóa draft-only chủ nhà, Tier 3, hoặc việc bỏ duyệt xóa/chia sẻ dữ liệu. Timeout chưa rõ kết quả không được tự gửi lại.

### Kết nối và thao tác Google Workspace (H018, chốt 2026-09-16)

- **REQ-GOOGLE-01:** Một điểm vào `/connect_google` (giữ alias `/connect-google`) hướng dẫn kết nối/cấp bổ sung quyền cho Gmail, Calendar, Drive, Docs, Sheets và Slides. Không giới hạn read-only; không hứa một màn hình OAuth nếu nhà cung cấp yêu cầu nhiều bước. Trạng thái phải phản ánh capability thực tế, không suy từ tài khoản ACTIVE.
- **REQ-GOOGLE-02:** Gmail hỗ trợ tìm/đọc, tạo/sửa nháp, gửi/trả lời kèm attachment được phép, quản lý nhãn và trạng thái thư/archive/trash. Calendar hỗ trợ tra cứu, free/busy, tạo/sửa/dời/hủy lịch và người tham dự; làm rõ timezone và tác động thông báo.
- **REQ-GOOGLE-03:** Drive hỗ trợ tìm/list file/thư mục, metadata, đọc/download/export có xác thực, upload, tạo thư mục, đổi tên/di chuyển/sao chép, xem/cấp/sửa/thu hồi chia sẻ, trash/restore. Không tự xóa vĩnh viễn hoặc đổi file thành public.
- **REQ-GOOGLE-04:** Docs hỗ trợ đọc các tab/bảng, tạo/chỉnh sửa nội dung; Sheets hỗ trợ đọc/ghi ô và công thức, append/clear vùng, tạo/sửa/xóa tab và định dạng; Slides hỗ trợ đọc/tạo/chỉnh sửa nội dung và thêm/sửa/xóa/sắp xếp slide. Bảo toàn nội dung ngoài vùng yêu cầu, kiểm tra lại thay đổi và xử lý xung đột phiên bản.
- **REQ-GOOGLE-05:** Link Drive/Docs/Sheets/Slides gửi qua Telegram phải được xử lý bằng tài khoản Google đã kết nối, gồm file private được chia sẻ riêng cho tài khoản đó. Không yêu cầu public file hoặc tải xuống gửi lại. Hỗ trợ selector tab/range/slide, phân trang, folder và shortcut/Shared Drive khi API cho phép; báo rõ phạm vi đọc thay vì giả vờ đọc toàn bộ.
- **REQ-GOOGLE-06:** Đọc file DOCX/XLSX/PPTX/PDF/TXT/CSV trên Drive bằng download có xác thực và bộ đọc phù hợp. Sửa/thay file binary qua công cụ định dạng sẵn có khi hỗ trợ; không tự chuyển đổi format hay hứa chỉnh sửa lossless mọi file. OCR scan và định dạng không hỗ trợ phải báo rõ. Không tự đưa Drive cá nhân vào RAG.
- **REQ-GOOGLE-07:** Mọi thao tác gắn caller/workspace từ host, đúng connection của user và quyền trên resource; nhiều account mơ hồ phải hỏi, không chọn account đầu tiên hay thử account người khác. Scope OAuth không thay thế phê duyệt hành động. Thu hồi quyền/ngắt kết nối chặn cả yêu cầu đang chờ; không tự chạy lại sau reconnect. Nội dung tài liệu không phải chỉ thị thực thi.
- **REQ-GOOGLE-08:** Lệnh kết nối hoặc yêu cầu Google cá nhân trong group được chuyển sang DM bằng nút/deep-link, giữ nguyên yêu cầu, gắn đúng người và workspace bằng token hết hạn/dùng một lần. Không gọi API đọc dữ liệu cá nhân hoặc lộ account/nội dung/kết quả trong group. Consent cá nhân không cấp quyền cho cả nhóm; shared-mailbox execution vẫn cần grant riêng.
- **REQ-GOOGLE-09:** Thực thi mutation có định danh yêu cầu bền vững, chống lặp khi retry/restart, phê duyệt theo tác động và kiểm chứng độc lập trên đúng account/resource. Phân biệt verified success, failed, partial và unknown; không retry write mù sau timeout. Gửi thành công không có nghĩa người nhận đã nhận/đọc. Quyền hiệu lực là giao của công cụ đã triển khai, OAuth scope, ACL resource, chính sách tổ chức/workspace và ủy quyền hành động; thiếu một lớp phải báo đúng nguyên nhân.

Nghiệm thu H018: kết nối mới/cấp thêm quyền; hai user và nhiều account cách ly; tìm và đọc link private các định dạng; thao tác đọc/ghi đủ sáu dịch vụ; gửi ngay không preview và draft-only không gửi; timeout/restart không gửi trùng; group tiếp tục đúng DM; chống replay/prompt injection; thu hồi quyền, kill switch, draft-only chủ nhà và Tier 3 vẫn chặn. Kiểm chứng độc lập qua Telegram thật và API, không chỉ mock. Spec chi tiết: `docs/superpowers/specs/2026-09-16-google-workspace-actions.md`; plan: `docs/superpowers/plans/2026-09-16-google-workspace-actions.md`. Hai file docs là tài liệu local theo D010; các yêu cầu ở đây đủ giữ phạm vi trên fresh clone.
